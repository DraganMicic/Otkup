...
public class SacuvajIStampajKontroler implements EventHandler<ActionEvent> {
	...
	public void handle(ActionEvent event) {
		...
		if(BrzUnosUlazaTab.getInstance().getTfKolicnaNeto().getText().equals("")) {
			Alert a = new Alert(AlertType.ERROR, "Niste uneli neto količinu!");
			BrzUnosUlazaTab.getInstance().getTfKolicnaNeto().requestFocus();
			a.show();
			return;
		}		
		...
		try {
			neto = Double.valueOf(BrzUnosUlazaTab.getInstance().getTfKolicnaNeto().getText());
		}catch (Exception e) {
			Alert ada = new Alert(AlertType.ERROR, "Neispravan format unosa (slovo/broj)!");
			BrzUnosUlazaTab.getInstance().getTfKolicnaNeto().requestFocus();
			ada.show();
		...
		if(Firma.getInstance().getTrenutnaGodina().cenaZaDatum(ulaz, datum)==null) {
			Alert a = new Alert(AlertType.ERROR, "Nema definisane cene za izabrani datum.");
			BrzUnosUlazaTab.getInstance().getDpDatum().requestFocus();
			a.show();
			return;
		}
		...
		if(BrzUnosUlazaTab.getInstance().isIzmenaUToku()) {
			...
			POSPrintUlaz.getInstance().stampajUlazNaPOS(uu);			
		}
