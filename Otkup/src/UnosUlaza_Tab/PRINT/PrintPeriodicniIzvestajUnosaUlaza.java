...
public class PrintPeriodicniIzvestajUnosaUlaza {

	private static PrintPeriodicniIzvestajUnosaUlaza instance;
	private int linija=0;
	private VBox glavniVB;
	private  ArrayList<Node> strane = new ArrayList<Node>();
	
	
	public static PrintPeriodicniIzvestajUnosaUlaza getInstance() {
		if(instance == null) {
			instance = new PrintPeriodicniIzvestajUnosaUlaza();
		}
		return instance;
	}
	 ...
	 public void stampajPeriodicniIzveštaj(LocalDate dPocetni, LocalDate dKrajnji) {
		 strane.clear();
		 linija=8;
		 ...
		 Label naslov = new Label("Periodični izveštaj ulaza");
		 ...
		 for(Ulaz u : Firma.getInstance().getTrenutnaGodina().ulaziZaPeriod(dPocetni, dKrajnji)) {
			 double kolicina = 0.0;
			 double vrednost = 0.0;
			 for(UnosUlaza uu : Firma.getInstance().getTrenutnaGodina().unosiUlazaZaPeriod(dPocetni, dKrajnji)) {
				 if(u.equals(uu.getUlaz())) {
					 kolicina += uu.getKolicinaNeto();
					 if(uu.getPrevoznik().equals(Firma.getInstance().getTrenutnaGodina().getLicno())) {
						 vrednost += uu.getKolicinaNeto() * (Firma.getInstance().getTrenutnaGodina().cenaZaDatum(u, uu.getDatum()).getCenaSaPrevozom() + uu.getProizvodjac().getCenaPlus());
					 }else {
						 vrednost += uu.getKolicinaNeto() * (Firma.getInstance().getTrenutnaGodina().cenaZaDatum(u, uu.getDatum()).getCenaBezPrevoza() + uu.getProizvodjac().getCenaPlus());
					 }
				 }
			 }
